...
function tValor($tValor)
{

	$tValor = number_format($tValor, 2, ".", ".");
	$tValor = str_replace(",", ".", $tValor);
	$tValor = str_replace(".", "", $tValor) / 100;
	$tValor  = number_format($tValor, 2, ',', '.');
...
foreach ($fatura as $mes_fat) {
	$query_valor = "SELECT * from historico_financeiro where id_unidade_consumidora = '$id' AND mes_faturado = '$mes_fat' AND data_pagamento_fatura is null AND id_acordo_parcelamento = '000000' AND status_estorno_tarifa = 'N' order by mes_faturado asc ";
	$result_valor = mysqli_query($conexao, $query_valor);
	while ($row_valor = mysqli_fetch_array($result_valor)) {
		...
		$rData = explode("/", $faturado_data); //2020       09
		...
		$t_tarifas = number_format($total_geral_tarifa, 2, ".", ".");

		$t_multas = number_format($total_multas_faturadas, 2, ".", ".");

		$t_juros = number_format($total_juros_faturados, 2, ".", ".");

		$t_servicos = number_format($total_servicos_requeridos, 2, ".", ".");

		$t_acordos = number_format($total_parcela_acordo, 2, ".", ".");

		$t_faturado = number_format($total_geral_faturado, 2, ".", ".");

		$query_up = "UPDATE historico_financeiro SET id_boleto_avulso = '$doc' where id_unidade_consumidora = '$id' AND mes_faturado = '$mes_fat' AND data_pagamento_fatura is null AND id_acordo_parcelamento = '000000' AND status_estorno_tarifa = 'N' ";

		$result_up = mysqli_query($conexao, $query_up);
	}
}

//echo tValor($total_geral_faturado) . ', ' . $t_faturado . ', ' . $t_acordos;
...
$dadosboleto["instrucoes2"] = "- Emitido pelo sistema SAAENET - www." . $home_page_saae;
