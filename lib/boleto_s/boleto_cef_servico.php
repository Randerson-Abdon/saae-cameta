...
$query_num = "select * from boleto_servico order by numero_boleto desc ";
...
//echo $id_requerimento . ', ' . $doc . ', ' . $codigo_barras . ', ' . $total_fatura2 . ', ' . $id_usuario_editor . ', ' . $data . ', ' . $vencimento;

$query_teste = "SELECT * from boleto_servico where id_requerimento = '$id_requerimento' AND  data_lancamento_boleto = '$data' ";
$result_teste = mysqli_query($conexao, $query_teste);
$linha_count = mysqli_num_rows($result_teste);

if ($linha_count > 0) {
	$query_delete = "DELETE FROM boleto_servico WHERE id_requerimento = '$id_requerimento' AND data_lancamento_boleto = '$data' ";
	$result_delete = mysqli_query($conexao, $query_delete);
}

$query_boleto = "INSERT INTO boleto_servico (id_requerimento, numero_boleto, id_unidade_consumidora, codigo_barras_boleto, valor_boleto, gerador_controle_boleto, data_lancamento_boleto, data_vencimento_boleto) values ('$id_requerimento', '$doc', '$id', '$codigo_barras', '$total_fatura2', '$id_usuario_editor', '$data', '$vencimento')";
