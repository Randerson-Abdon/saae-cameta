<bady>
  ...
  if (isset($_POST['processar'])) {
    ...
    $arquivo = "../include/RET_$local/$nome_retorno.txt";

    $arquivo_destino = "../include/PROCESSADO_RET_$local/$pasta_banco/$pasta_data/$nome_retorno.txt";

    if (file_exists("../include/PROCESSADO_RET_$local/$pasta_banco/$pasta_data/$nome_retorno.txt")) {
    ...
    if (($result_az == '') || ($result_g == '')) {
      ...
      @mkdir('../include/PROCESSADO_RET_' . $local . '/' . $pasta_banco . '/' . $pasta_data, 0777, true);
